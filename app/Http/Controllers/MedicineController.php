...
class MedicineController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return app('datatables')->of($medicines)
                ->addColumn('actions', function ($medicine) {
                    $html = '
                        <a href="' . $medicine->edit_route . '" class="btn btn-sm btn-default btn-text-primary btn-hover-primary btn-icon mr-2" title="Edit details">
    ...
    public function edit($id = 0)
    {
        ...
        if (Route::currentRouteName() == 'settings.medicines.edit') {
    ...
    public function store(Request $request)
    {
        $rules = [
            ...
            'expiry' => ['required', 'string', 'min:1', 'max:255'],
        ];
        ...
        return redirect()->route('settings.medicines');
    }
}
